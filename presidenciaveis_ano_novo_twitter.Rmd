...
A próxima etapa foi executar os procedimentos de análises relacionados à importância dos termos na definição se uma mensagem é positiva ou negativa. Usou-se duas abordagens para isso: a TF-IDF que mostra a importância dos termos únicos para cada uma classes (positiva ou negativa) e a árvore de decisão que indica como as palavras se organizam hierarquicamente na definição da classe da mensagem. Ao se realizar essa última etapa perdeu-se mais um candidato. A árvore de decisão não conseguiu ser montada para o candidato Moro, possivelmente pela pouca quantidade de mensagens relevantes para essa classificação.


## Análise do Bolsonaro

Bolsonaro foi o candidato mais associados hashtags com um número muito mais elevado do que qualquer outro candidato, incluindo Lula que está na liderança da corrida presidencial de acordo com as pesquisas mais recentes. Como veremos, porém, isso não necessariamente é uma boa notícia para o presidente.
...
tags_postivas<- analise_bolsonaro$hashtag[c(2,3,8,9,10)]
tags_negativas<- analise_bolsonaro$hashtag[c(1,7)]

analise_bolsonaro%>%
  filter(hashtag %in% c(tags_postivas, tags_negativas)) %>%
  mutate(hashtag = reorder(hashtag, quantidade)) %>%
  mutate(classe = ifelse(hashtag %in% tags_postivas,"Positivo", "Negativo")) %>%
  ggplot()+
